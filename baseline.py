...
def reorderbands(srcpath, dstpath, bandlist, deletesource=False):
    ...
    copyfile = driver.Create(dstpath, shape[1], shape[0], numbands, gdal.GDT_Byte)
    for bandnum in range(1, numbands + 1):
...
def pretrain(args):
    ...
    print("Pretrain")
    assert (
        args.sardir is not None
        and args.labeldir is not None
        and args.maskdir is not None
        and args.sarprocdir is not None
    )
    ...
    combodf = pd.DataFrame(columns=["opticalimage", "sarimage", "label", "group"])
...
training:
  ...
  verbose: true
"""
    yamlcontents = yamlcontents.replace("$OPTICALTRAINCSV", args.opticaltraincsv)
    yamlcontents = yamlcontents.replace("$OPTICALVALIDCSV", args.opticalvalidcsv)
    yamlcontents = yamlcontents.replace("$MODELDIR", args.modeldir)
...
def pretest(args):
    ...
    for i, (sarpath, sarprocpath) in tqdm.tqdm(enumerate(zip(sarpaths, sarprocpaths)), total=len(sarpaths)):
        ...
        testdf = testdf.append({"image": sarprocpath}, ignore_index=True)
